<nav class="bg-white sticky top-0 z-50 shadow-sm">
    ...
    <div id="mobile-menu" class="hidden md:hidden bg-white border-t border-gray-100 shadow-lg absolute w-full">
        <div class="px-6 py-4 flex flex-col gap-4">
            <a href="{{ url('/') }}" class="text-base font-medium {{ request()->is('/') ? 'text-[#991B1B]' : 'text-gray-600 hover:text-[#991B1B]' }}">Home</a>
            ...
            @auth
                @if (auth()->user()->role === 'member')
                    <a href="{{ route('member.dashboard') }}" class="text-base font-medium {{ request()->routeIs('member.dashboard') ? 'text-[#991B1B]' : 'text-gray-600 hover:text-[#991B1B]' }}">Dashboard</a>
                    <a href="{{ route('member.profile') }}" class="text-base font-medium {{ request()->routeIs('member.profile') ? 'text-[#991B1B]' : 'text-gray-600 hover:text-[#991B1B]' }}">Profile</a>
                @else
                    <a href="{{ route('dashboard') }}" class="text-base font-medium text-gray-600 hover:text-[#991B1B]">Dashboard</a>
                @endif
            @else
                <a href="{{ route('login') }}" class="text-base font-medium text-gray-600 hover:text-[#991B1B]">Login</a>
