<header class="h-20 border-b border-gray-100 px-4 sm:px-8 flex items-center justify-between gap-4 bg-white shrink-0">
    ...
    <form action="{{ route('reservations.index') }}" method="GET" class="relative hidden sm:block w-96 max-w-full">
        ...
        <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari tamu atau reservasi..." class="w-full pl-10 pr-4 py-2 bg-gray-50 border-transparent rounded-full focus:bg-white focus:border-gray-200 focus:ring-0 text-sm">
    </form>
    ...
    <div class="flex items-center gap-5 text-gray-500">
        ...
        <div class="w-8 h-8 rounded-full bg-gray-200 overflow-hidden ml-2 border border-gray-200">
            <img src="https://ui-avatars.com/api/?name={{ urlencode(auth()->user()->name ?? 'Pengguna') }}&background=F3F4F6&color=A31621" alt="Profil" class="w-full h-full object-cover">
        </div>
    </div>
</header>
